...
fn no_tls(config: Config) -> NoTlsConnectionProvider {
    NoTlsConnectionProvider {
        config,
    }
}

pub fn create_connection_provider(config: Config, ca_file: Option<&str>) -> Result<Box<dyn ConnectionProvider>, Error> {
...
pub(crate) mod tls_openssl {
    ...
    use tokio_postgres::{Config, Client};
...
pub(crate) mod tls_rustls {
    ...
    pub fn new(config: Config, ca_file: &str) -> Result<Box<dyn ConnectionProvider>, Error> {
        ...
        root_store.add_pem_file(reader)
